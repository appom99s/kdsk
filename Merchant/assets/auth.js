const KADOSK_AUTH = (function () {
  ...
  function ecrireCookieTemporaire(nom, valeur, maxAge) {
    document.cookie = nom + "=" + encodeURIComponent(valeur || "") + "; Domain=.kadosk.com; Path=/; Max-Age=" + maxAge + "; Secure; SameSite=Lax";
  }
  ...
  function marquerVerification2FA() {
    try {
      ecrireCookieTemporaire(VERIF_2FA_STORAGE_KEY, "1", 12 * 60 * 60);
    } catch (erreur) {
  ...
  async function ecrireTokens(tokens) {
    ...
    const reponse = await fetch(config().siteBaseUrl + "/_functions/sessionStart", {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json", "X-Kadosk-Device-Id": btoa(unescape(encodeURIComponent([navigator.userAgent, navigator.platform, screen.width + "x" + screen.height].join("|")))).slice(0, 160) },
    ...
    tokensTemporaires = null;
    ecrireCookieTemporaire(SESSION_BRIDGE, session.sessionToken, 30 * 24 * 60 * 60);
    ecrireCookieTemporaire(SESSION_HINT, "1", 30 * 24 * 60 * 60);
  }
  ...
  async function effacerTokens() {
    ...
    tokensTemporaires = null;
    ecrireCookieTemporaire(PKCE_STORAGE_KEY, "", 0);
    ecrireCookieTemporaire(STATE_STORAGE_KEY, "", 0);
    ecrireCookieTemporaire(SESSION_HINT, "", 0);
    ecrireCookieTemporaire(SESSION_BRIDGE, "", 0);
  ...
  async function demarrerAutorisationMembre(sessionToken, cheminRedirectionPersonnalise) {
    ...
    ecrireCookieTemporaire(PKCE_STORAGE_KEY, verifier, 600);
    ecrireCookieTemporaire(STATE_STORAGE_KEY, etat, 600);
  ...
  async function renouvelerAccessToken() { return ""; }
