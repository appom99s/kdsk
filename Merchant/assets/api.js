const KADOSK_API = (function () {
  ...
  async function executerAppel(nom, methode, corps, accessToken, dejaReessaye, estPublic) {
    if (!estPublic) {
      let reponseSession;
      try {
        reponseSession = await fetch(KADOSK_CONFIG.siteBaseUrl + "/_functions/sessionInvoke", {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json", "X-Kadosk-Session": KADOSK_AUTH.lireSessionBridge() },
          body: JSON.stringify({ name: nom, method: methode, body: corps })
        });
      } catch (erreurReseau) {
        throw new Error("ERREUR_RESEAU");
      }
      const texteSession = await reponseSession.text();
      let donneesSession = {};
      try { donneesSession = texteSession ? JSON.parse(texteSession) : {}; } catch (erreur) { donneesSession = {}; }
    ...
    if (reponse.status === 401 && !dejaReessaye && !estPublic) {
      try {
        const nouveauToken = await KADOSK_AUTH.renouvelerAccessToken();
        return executerAppel(nom, methode, corps, nouveauToken, true, false);
      } catch (erreurRenouvellement) {
    ...
    if (!reponse.ok) {
      ...
      erreur.detail = donnees.debug || donnees.details || null;
  ...
  return {
    ...
    saveInvoiceTemplate: (parametres) => appeler("invoiceTemplate", "POST", parametres),
    ...
    getMerchantGrowth: () => appeler("merchantGrowth", "GET"),
    saveMerchantGrowth: (settings) => appeler("merchantGrowth", "POST", settings),
    getGiftCardDetail: (giftCardId) => appeler("giftCardDetail?giftCardId=" + encodeURIComponent(giftCardId), "GET"),
    changeGiftCardStatus: (giftCardId, action, reason) => appeler("giftCardStatus", "POST", { giftCardId, action, reason }),
    ...
    createOrder: (items, buyerEmail, recipientName, recipientEmail, message, forSelf, buyerName, buyerPhone, saveCustomerData) =>
      appelerPublic("createOrder", "POST", { items, buyerEmail, recipientName, recipientEmail, message, forSelf, buyerName, buyerPhone, saveCustomerData: saveCustomerData === true }),
...
// Les modules de navigation/guard testent explicitement window.KADOSK_API avant
// de lancer l'identification du rôle. Une déclaration top-level `const` ne crée
// pas de propriété sur window : sans cette affectation, aucun appel chromeInfo
// n'était jamais envoyé et le menu restait bloqué sur son état minimal.
window.KADOSK_API = KADOSK_API;
