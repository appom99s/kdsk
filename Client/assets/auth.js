const KADOSK_AUTH = (function () {
  ...
  function ecrireCookie(nom, valeur, maxAge, sameSite) {
    document.cookie = nom + "=" + encodeURIComponent(valeur || "") + "; Domain=.kadosk.com; Path=/; Max-Age=" + maxAge + "; Secure; SameSite=" + (sameSite || "Lax");
  }
  function lireCookie(nom) {
    const prefixe = nom + "=";
    const partie = document.cookie.split(";").map((v) => v.trim()).find((v) => v.startsWith(prefixe));
    return partie ? decodeURIComponent(partie.slice(prefixe.length)) : "";
  }
  ...
  async function ecrireTokens(tokens) {
    ...
    const reponse = await fetch(config().siteBaseUrl + "/_functions/sessionStart", {
      method: "POST", credentials: "include", headers: { "Content-Type": "application/json", "X-Kadosk-Device-Id": btoa(unescape(encodeURIComponent([navigator.userAgent, navigator.platform, screen.width + "x" + screen.height].join("|")))).slice(0, 160) },
      body: JSON.stringify({ accessToken: tokens.access_token, refreshToken: tokens.refresh_token, expiresIn: tokens.expires_in, clientId: config().clientId })
    });
    if (!reponse.ok) {
      const erreurSession = await reponse.json().catch(() => ({}));
      tokensTemporaires = null;
      throw new Error(erreurSession.error || "SESSION_SERVEUR_INDISPONIBLE");
    }
    tokensTemporaires = null;
    const session = await reponse.json().catch(() => ({}));
    if (!session.sessionToken) throw new Error("SESSION_SERVEUR_INDISPONIBLE");
    ecrireCookie(SESSION_BRIDGE, session.sessionToken, 30 * 24 * 60 * 60, "Strict");
    ecrireCookie(SESSION_HINT, "1", 30 * 24 * 60 * 60, "Strict");
  }
  ...
  async function effacerTokens() {
    ...
    tokensTemporaires = null;
    ecrireCookie(PKCE_STORAGE_KEY, "", 0);
    ecrireCookie(STATE_STORAGE_KEY, "", 0);
    ecrireCookie(SESSION_HINT, "", 0, "Strict");
    ecrireCookie(SESSION_BRIDGE, "", 0, "Strict");
  ...
  async function demarrerAutorisationMembre(sessionToken, cheminRedirectionPersonnalise) {
    ...
    ecrireCookie(PKCE_STORAGE_KEY, verifier, 600);
    ecrireCookie(STATE_STORAGE_KEY, etat, 600);
...
const KADOSK_BUYER_SESSION = (function () {
  ...
  function attendre(delaiMs) {
    ...
    return new Promise((resolve) => {
      let termine = false;
      const finir = () => { if (termine) return; termine = true; document.removeEventListener("kadosk:buyer-logged-in", recevoir); resolve(lire()); };
      const recevoir = () => finir();
      document.addEventListener("kadosk:buyer-logged-in", recevoir, { once: true });
      setTimeout(finir, Math.max(1000, Number(delaiMs) || 8000));
    });
  }
  return { definir, lire, effacer, attendre, deviceId };
})();
window.KADOSK_BUYER_SESSION = KADOSK_BUYER_SESSION;
